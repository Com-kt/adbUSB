...
class MainActivityViewModel : ViewModel() {
    ...
    private val _adbCommands = listOf(
        ...
        AdbCommand("抓取崩溃缓冲区的日志", "logcat -b crash"),
        ...
        AdbCommand("列出所有应用包名", "pm list packages | wc -l"),
        AdbCommand("pm list packages [选项] [过滤字符]", "pm list packages"),
        AdbCommand("仅列出第三方应用包名", "pm list packages -3 | wc -l"),
        AdbCommand("仅列出系统预装应用包名", "pm list packages -s | wc -l"),
        AdbCommand("仅列出被禁用的应用包名", "pm list packages -d | wc -l"),
        AdbCommand("仅列出处于启动状态的应用包名", "pm list packages -e | wc -l"),
