...
#define HARD_ASSERT(ptr, exit_code) if (!(ptr)) { std::exit(exit_code); }
...
Java_com_adb_kitty_compose_data_NativeLibs_nativeVerify(JNIEnv *env, jobject thiz, jobject context) {
    ...
    HARD_ASSERT(getApkContentsSignersMd, 114);
    ...
    HARD_ASSERT(signatureObj, 115);

    jclass signatureClz = env->GetObjectClass(signatureObj);
    HARD_ASSERT(signatureClz, 116);
    ...
    HARD_ASSERT(messageDigestClz, 119);
    ...
    HARD_ASSERT(getInstanceMd, 120);
    
    jobject messageDigest = env->CallStaticObjectMethod(messageDigestClz, getInstanceMd, env->NewStringUTF(_XOR_({ 0x09, 0x12, 0x1b, 0x77, 0x68, 0x6f, 0x6c }).c_str()));
    HARD_ASSERT(messageDigest, 121);
    
    jmethodID digestMd = env->GetMethodID(messageDigestClz, 
        _XOR_({ 0x3e, 0x33, 0x3d, 0x3f, 0x29, 0x2e }).c_str(), 
        _XOR_({ 0x72, 0x01, 0x18, 0x73, 0x01, 0x18 }).c_str());
    HARD_ASSERT(digestMd, 122);
    
    auto currentHashBytes = (jbyteArray)env->CallObjectMethod(messageDigest, digestMd, certificateBytes);
    HARD_ASSERT(currentHashBytes, 123);
