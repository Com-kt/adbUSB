...
class MainActivityViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val _appCommands = listOf(
        ...
        AppCommand("以system身份+全局挂载空间调用本地 Shell , 该指令由app提供", "neko su system -M -c "),
        AppCommand("以adb身份+全局挂载空间调用本地 Shell , 该指令由app提供", "neko su adb -M -c "),
        AppCommand("以shell身份+全局挂载空间调用本地 Shell , 该指令由app提供", "neko su shell -M -c "),
